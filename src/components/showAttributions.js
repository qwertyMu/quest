...
const ShowAttributions = props => {
  ...
  function ListAttributions({ pk }) { //Aug 2022 - This is a working prototype of the query behavious i'm looking for. 
    ...
    if (data !== undefined){
      console.log(data);
      return <div>
        {data.listAttributions.items.map(({ nominal, organisation, attribution, file_name, datetime_added, exhibit }, index) => (
          <AttributesCard 
            key={index}
            Id={nominal}
            Nominal={nominal}
  ...
  return (
    <div style={{display: "inline-grid", inlineSize: "max-content", width: "100%"}}>
          <h2>🚀 Quest has ATTRIBUTED the following identifiers to <b>{pk}</b></h2>
